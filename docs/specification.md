...
モデルの配信元によって、Dockerイメージへの含め方が異なる。

#### 配信元ごとの推奨方式

| 配信元 | 認証 | 推奨方式 | 理由 |
|--------|------|---------|------|
| HuggingFace (ungated) | 不要 | ビルド時DL (`RUN wget`) | 認証不要。イメージに含めればコールドスタートが速い |
| HuggingFace (gated) | HF_TOKEN | ビルド時DL (`--mount=type=secret`) | BuildKit secretでトークンをレイヤーに残さず安全にDL可能 |
| Civitai (Early Access) | CIVITAI_API_TOKEN | 起動時DL (entrypoint) | API仕様変更リスクあり。環境変数でトークンを渡す方が運用しやすい |

#### ビルド時DL vs 起動時DL 比較

| 観点 | ビルド時DL | 起動時DL |
|------|----------|---------|
| イメージサイズ | +6〜7GB（push/pullが遅い） | 軽量 |
...
Network Volumeをマウントすれば、起動時DLでも2回目以降はDLをスキップしコールドスタートの遅延を回避できる。

#### ungatedモデルの場合（ビルド時DL）

```dockerfile
RUN mkdir -p /ComfyUI/models/checkpoints && \
    wget -q https://huggingface.co/org/model/resolve/main/model.safetensors \
    -O /ComfyUI/models/checkpoints/model.safetensors
```

#### gatedモデルの場合（BuildKit secret）
...
`entrypoint.sh` でコンテナ起動時にダウンロードする。

```bash
if [ ! -f "$CKPT_PATH" ]; then
    wget -q "https://civitai.com/api/download/models/${MODEL_ID}?token=${CIVITAI_API_TOKEN}" \
        -O "$CKPT_PATH"
fi
```
...
| 変数名 | 必要な場合 | 説明 |
|--------|----------|------|
| `CIVITAI_API_TOKEN` | Civitaiモデル使用時 | コンテナ起動時のチェックポイントDLに使用 |
| `HF_TOKEN` | HuggingFace gatedモデル使用時 | ビルド時のチェックポイントDLに使用（BuildKit secret経由） |
