...
Momoiro Pony v1.5 によるアニメ/イラスト画像生成API（RunPod Serverless）

## 概要

テキストプロンプトからアニメ・イラスト画像を生成するAPI。ComfyUIバックエンドでMomoiro Pony v1.5（SDXLベース）モデルを使用し、RunPod Serverless上で動作する。
...
| コンポーネント | 詳細 |
|--------------|------|
| 生成モデル | Momoiro Pony v1.5 (SDXL, ~6.5GB, public mirror) |
| CLIP Skip | 2 |
